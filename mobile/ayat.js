...
function splitReadableParagraphs(text){
  ...
  for(const sentence of sentences){
    ...
    if(current&&candidate.length>430){out.push(current);current=sentence}else current=candidate;
  }
...
function renderCore(){
  ...
  $('detailTitle').textContent=`${name} Ayat ${n}`;
  $('detailMeta').textContent=`${data.arti} · ${typeLabel(data.tempatTurun)} · ${data.jumlahAyat} ayat`;
  $('detailArabicName').textContent=glyph(sid);
  ...
  setNav(n);setupAudio();restoreBookmark();icons();
}
...
function setNav(n){
  if(n>1){prevHref=`./ayat.html?surah=${sid}&ayat=${n-1}`;$('prevAyatLabel').textContent=`${data.namaLatin} ${n-1}`}
    ...
    const last=data.suratSebelumnya.jumlahAyat||1;prevHref=`./ayat.html?surah=${data.suratSebelumnya.nomor}&ayat=${last}`;$('prevAyatLabel').textContent=`${data.suratSebelumnya.namaLatin} ${last}`
  }
  ...
  $('prevSticky').title=$('prevAyatLabel').textContent?`Sebelumnya: ${$('prevAyatLabel').textContent}`:'Ayat sebelumnya';
  $('nextSticky').title=$('nextAyatLabel').textContent?`Berikutnya: ${$('nextAyatLabel').textContent}`:'Ayat berikutnya';
}
...
async function loadTafsir(){
  try{const r=await fetch(`${API}/tafsir/${sid}`);if(!r.ok)throw new Error('tafsir');const j=await r.json();const taf=(j?.data?.tafsir||[]).find(x=>x.ayat===verse.nomorAyat);const parts=splitReadableParagraphs(taf?.teks||'Tafsir belum tersedia untuk ayat ini dari sumber API.');$('tafsirText').innerHTML=parts.map(p=>`<p>${p}</p>`).join('')}
  catch(e){$('tafsirText').innerHTML='<p>Tafsir sedang tidak dapat dimuat. Silakan coba kembali.</p>'}
}

function bindControls(){
  $('detailJump').onchange=e=>{const n=+e.target.value;if(n)location.href=`./ayat.html?surah=${sid}&ayat=${n}`};
  $('sectionJump').onchange=e=>document.getElementById(e.target.value)?.scrollIntoView({behavior:'smooth',block:'start'});
  $('prevSticky').onclick=()=>{if(prevHref)location.href=prevHref};$('nextSticky').onclick=()=>{if(nextHref)location.href=nextHref};
  $('saveAyatBtn').onclick=saveAyat;$('shareAyatBtn').onclick=shareAyat;
  const sections=['arti','tafsir','poin-penting','asbabun-nuzul','faq'];
  const syncSection=()=>{let active='arti';for(const id of sections){const el=document.getElementById(id);if(el&&el.getBoundingClientRect().top<=205)active=id;else if(el)break}$('sectionJump').value=active};
  window.addEventListener('scroll',syncSection,{passive:true});syncSection();
}

async function init(){
  bindControls();icons();
  try{const r=await fetch(`${API}/surat/${sid}`);if(!r.ok)throw new Error('surat');const j=await r.json();data=j.data;verse=data.ayat.find(v=>v.nomorAyat===requestedAyat)||data.ayat[0];renderCore();loadTafsir()}
  catch(e){$('detailTitle').textContent='Data ayat belum dapat dimuat';$('detailMeta').textContent='Silakan periksa koneksi dan coba lagi.';$('detailArab').textContent='';$('detailLatin').textContent='';$('artiText').textContent='Data ayat belum tersedia.';$('tafsirText').innerHTML='<p>Tafsir belum dapat dimuat.</p>'}
}
